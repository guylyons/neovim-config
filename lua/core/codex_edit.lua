...
-- Codex must wrap its replacement between these marker lines. Extracting only the
-- delimited block means any stray prose it emits is discarded instead of being
-- written into the buffer, and a reply with no block is rejected outright.
local REPLACEMENT_OPEN = "<<<CODEX_REPLACEMENT"
local REPLACEMENT_CLOSE = ">>>CODEX_REPLACEMENT"

---Extract the replacement lines from a Codex reply.
---@param raw string
...
function M.edit(opts)
	...
	vim.system({
		...
		vim.schedule(function()
			...
			if not lines then
				stop_spinner(spinner)
				notify("Codex did not return a wrapped replacement; buffer left unchanged.", vim.log.levels.ERROR)
				return
			end
