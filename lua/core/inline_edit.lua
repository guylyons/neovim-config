--- Shared backend for inline range-edit integrations.
--- Builds an edit prompt from buffer context, runs a configured CLI, and applies
--- the returned replacement to the buffer. `create()` binds this logic to one CLI
--- so thin wrappers (ai_edit, codex_edit) only supply how to run their command.
--- 2026
local M = {}
...
-- the delimited block means any stray prose it emits is discarded instead of being
-- written into the buffer, and a reply with no block is rejected outright.
local REPLACEMENT_OPEN = "<<<INLINE_REPLACEMENT"
local REPLACEMENT_CLOSE = ">>>INLINE_REPLACEMENT"
...
---@return string
local function build_prompt(instruction, selected_text)
...
---Build a range-edit integration bound to one CLI.
---@param config { title: string, vendor: string, executable: string, commands: table, run: fun(prompt: string, done: fun(result: table)) }
---@return table
function M.create(config)
	local namespace = vim.api.nvim_create_namespace("inline_edit:" .. config.title)

	---Show a scheduled Neovim notification for this integration.
	...
	---Render an inline spinner on its own line above the range while the CLI runs.
	---@param bufnr integer
	...
	local function apply_replacement(bufnr, edit_range, start_mark, end_mark, lines)
		...
		notify(("Applied %s edit."):format(config.vendor))
	end

	---Run an edit for the command-provided range.
	---@param opts table User command callback options.
	local function edit(opts)
		...
		if vim.fn.executable(config.executable) ~= 1 then
			notify(("The %s CLI is not available on PATH."):format(config.executable), vim.log.levels.ERROR)
			return
		end
		...
		notify(("Asking %s..."):format(config.vendor))

		config.run(build_prompt(instruction, text), function(result)
			vim.schedule(function()
				if not vim.api.nvim_buf_is_valid(bufnr) then
					stop_spinner(spinner)
					return
				...
				if result.code ~= 0 then
					stop_spinner(spinner)
					local message = vim.trim(result.stderr or result.output or (config.vendor .. " command failed."))
					notify(message, vim.log.levels.ERROR)
					return
				end

				local lines = M.parse_response(result.output)
				if not lines then
					stop_spinner(spinner)
					notify(
						("%s did not return a wrapped replacement; buffer left unchanged."):format(config.vendor),
	...
	---Register the line and range user commands for this integration.
	local function setup()
		vim.api.nvim_create_user_command(config.commands.line.name, function(command)
			local line = vim.api.nvim_win_get_cursor(0)[1]
			edit({ args = command.args, line1 = line, line2 = line, range = 0 })
		end, {
			nargs = "+",
			desc = config.commands.line.desc,
		})

		vim.api.nvim_create_user_command(config.commands.edit.name, edit, {
			nargs = "+",
			range = true,
			desc = config.commands.edit.desc,
		})
	end
