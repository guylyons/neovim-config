...
- `init.lua`: entry point; loads settings, registers plugins through native `vim.pack`, then loads plugin config and keymaps.
- `lua/settings.lua`: core editor options, completion options, filetype autocommands, and folding behavior.
- `lua/plugins/*.lua`: one file per plugin or feature area (`treesitter`, `lspconfig`, `conform`, etc.).
- `lua/keybindings.lua`: custom mappings.
...
- Prefer small, focused modules under `lua/plugins/` with descriptive lowercase names (for example, `treesitter.lua`, `lspconfig.lua`).
...
- Run `:checkhealth` and `:checkhealth vim.lsp` after LSP/plugin changes.
